...
def load_and_evaluate(checkpoint_path, batch_size=64, device="cuda"):
    ...
    # 2. Load Model
    print("Loading Model...")
    # load_from_checkpoint automatically restores hyperparameters (beta, z_size)
    # if self.save_hyperparameters() was called in LitBetaVae.__init__
    model = LitBetaVae.load_from_checkpoint(checkpoint_path)
    
    model.eval()
    model.to(device)
    
    # Create output directory
    # Structure: checkpoints/beta_vae_b1.0/evaluation_post_hoc/
    ...
    with open(os.path.join(eval_dir, "linear_probe_results.txt"), "w") as f:
        f.write(f"Linear Probe Results for {ckpt_name}\n")
        f.write("=============================================\n")
        f.write(f"{'Factor':<15} | {'Accuracy':<10}\n")
        f.write("-" * 30 + "\n")
...
if __name__ == "__main__":
    ...
    load_and_evaluate(args.ckpt, batch_size=args.batch_size, device=device)